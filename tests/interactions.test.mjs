...
function mount(matches = true) {
  ...
  const source = readFileSync(
    new URL("../src/hooks/useSurfaceProbe.ts", import.meta.url),
    "utf8",
  );
  runInNewContext(
  ...
  return {
    ...
    move: (pointerType = "mouse") =>
      listeners.get("pointermove")({ pointerType, clientX: 980, clientY: 290 }),
    reduce: () => {
      media.matches = false;
      mediaChange();
    },
...
test("inspection lens tracks local coordinates and cancels when reduced motion changes", () => {
  const ui = mount();
  ui.move();
  ui.flush();
  assert.equal(ui.element.dataset.probing, "true");
  assert.equal(parseFloat(ui.styles.get("--probe-x")), 980);
  assert.equal(parseFloat(ui.styles.get("--probe-y")), 290);
  ui.reduce();
  assert.equal(ui.frames.size, 0);
